...
public class Main extends Application {
	...
	private LineChart drawCountryGraph() {
		...
		for (int i = 0; i < dates.size(); i++) {
			countryGraph[0][i] = dates.get(i).date.substring(0, dates.get(i).date.length() - 5);
			; // Date minus the year
...
//		add(new XYChart.Data<String, Number>(unknown.ageSpan, unknown.totalCases));
	...
	private HBox drawStateGraph() {
		...
		submitBtn.setOnAction(e -> changeStateGraph(statesComboBox.getValue()));
		
		
		//patients.
		
		//List<Patient> patientList = Arrays.asList(patients);
		//patientList.stream().filter(predicate)
			
		
		// center for state
		VBox leftCenterBoxTop = new VBox(statesComboBox);
		VBox leftCenterBoxBottom = new VBox(submitBtn);
	...
	public void changeStateGraph(String stateCodeState) {
